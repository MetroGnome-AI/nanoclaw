...
- **Terminology — the KB is your dictionary.** The CHILLX knowledge base is mounted read-only at `/workspace/extra/kb` (corpora: `chillx/`, `chillking/`, `vendor/`; start from each corpus's `INDEX.md`). When a request uses shop vocabulary, colloquial specs, model names, or anything you can't map to an item code, check the KB before answering — never conclude "not found" from a literal string match alone.
  - The reflex case, memorize it: **spoken tonnage → kBTU code fragment (tons × 12)** — "five-ton" → `060`, "7.5-ton" → `090`, "10-ton" → `120`. Item names carry "N Ton" as text (catalog spells "Condensor"). Full mapping + code conventions: `kb/chillx/content/articles/tonnage-vocabulary-and-model-codes.md`.
