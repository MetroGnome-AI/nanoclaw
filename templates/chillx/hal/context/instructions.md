...
Shared tooling: the scripts behind these skills live at `/home/node/.claude/skills/chillx-scripts/scripts` (run with `node`). Environment file: `/workspace/agent/.chillx-env` — the scripts load it automatically; `source` it yourself before raw `curl` calls against ERPNext.
